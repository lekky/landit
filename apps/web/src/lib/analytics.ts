/**
 * What Land The Trick tells its analytics services, and what it deliberately
 * does not (§6.8).
 *
 * **There are two of them since 2026-09-06**: PostHog, which everything below
 * describes, and Google Analytics 4, which is configured at the foot of this
 * file. PostHog is unchanged by that addition — same options, same counting,
 * same promises — and the GA section carries its own argument for why a second
 * one exists and why it is cookieless. Everything in between, most of all the
 * event catalogue and the rule about what may travel with an event, applies to
 * both: `capture()` sends every event to whichever is configured.
 *
...
/**
 * The PostHog project key, or empty when PostHog is switched off.
 *
 * `analyticsKey`, `analyticsHost` and `analyticsOptions` are **PostHog's half**
 * and keep their unqualified names because they predate the second service and
 * are pinned under them in the test. GA's equivalents are `gaMeasurementId`
 * and `gaOptions`; neither service's switch says anything about the other, so
...
/**
 * The second analytics service, added 2026-09-06 (Rachid, in chat).
 *
 * **This reverses the 2026-08-21 decision that PostHog would be the only one**
 * (plan §1), and it is recorded here rather than slipped in: Cloudflare Web
 * Analytics was dropped that day *specifically* to get to one analytics
 * processor, and this puts a second one back. The reason is that the owner
 * already works in Google Analytics day to day and wants these numbers in that
 * interface. PostHog is unchanged and keeps the daily-unique count, which is
 * the one thing the configuration below cannot produce.
 *
 * ## Why it is cookieless, and what that costs
 *
 * `client_storage: 'none'` is the whole reason this can ship without a consent
 * banner. Nothing is stored on or read from the rider's device, so PECR
 * regulation 6 and ePrivacy Article 5(3) do not bite at all — the same footing
 * PostHog is on, and the reason `/legal/cookies` can still say there is no
 * cookie pop-up.
 *
 * **GA4 with its normal `_ga` cookie was considered and is not available to
 * this product.** It would need consent in both the UK and the EU: the UK's new
 * statistical-purposes exception (DUAA 2025 Sch A1 para 5, ICO guidance of
 * 2026-04-29) is drawn to exclude user-level tracking, cross-site tracking and
 * advertising measurement, and a two-year per-device client id is user-level
 * tracking by design. Consent is then the wrong tool anyway: UK GDPR Article 8
 * puts the digital-consent age at 13, so most riders here cannot lawfully give
 * it, and the ICO's Children's Code says this kind of collection should be off
 * by default for everyone under 18. A banner our own users cannot answer buys
 * nothing.
 *
 * Feeding GA a stable client id of our own was considered too, and refused: it
 * would need either device storage (consent again) or a fingerprint derived
 * from browser signals, which is caught by the same rules and is a worse thing
 * to do to a child than the cookie was.
 *
 * **The cost is that GA mints a fresh client id on every page load.** Its
 * "users" figure is therefore page loads, not people, and no funnel it draws
 * across two page loads is real. Event counts and traffic are sound; anything
 * shaped like retention is not, and PostHog is where that question gets asked.
 *
 * ## What this file cannot do
 *
 * Three settings live in the GA property and **cannot be set from this
 * repository**. All three fail silently, which is why they are written down:
 *
 *  - **Enhanced measurement must be OFF.** It is on by default for a new
 *    property and it is GA's autocapture — outbound clicks, scroll, file
 *    downloads, video engagement, and *site search*, which reads the query
 *    string. On this product that would send words a child typed to Google,
 *    which is exactly what `autocapture: false` refuses above and exactly what
 *    the rule on this catalogue forbids. This is the highest-risk setting in
 *    the whole integration.
 *  - **Google-products data sharing must be OFF**, along with Google signals
 *    and ad personalisation. The two options below ask for that from the
 *    client; the property-level toggles are what actually decide it.
 *  - **Custom dimensions have to be registered** before any property here shows
 *    up in a report. Event counts appear on their own; `sport`, `tier`,
 *    `source` and the rest are invisible until each is added in the GA UI. The
 *    free tier allows 50 and this catalogue uses roughly 20.
 *
 * **Loading gtag.js means Google sees the request.** The script comes from
 * `googletagmanager.com`, so Google receives a rider's IP address and user
 * agent on every page load however this is configured. That is inherent to
 * client-side GA rather than a setting we chose, and it is the honest reason
 * the cookie policy now names Google rather than saying "our analytics
 * service".
 */

/** The measurement id (`G-…`), or empty when GA is switched off. */
...
/**
 * Consent Mode defaults, set **before** `config` and before gtag.js has run.
 *
 * **This, and not `client_storage`, is what stops GA4 writing a cookie.**
 * `client_storage: 'none'` is a Universal Analytics parameter; GA4 accepts it
 * without complaint and ignores it. That was checked rather than assumed, in a
 * real browser on 2026-09-06: with `client_storage: 'none'` set and nothing
 * else, cookies were cleared, the page reloaded, and GA4 wrote a fresh `_ga`
 * and `_ga_<id>` pair with a new client id. The parameter is not set any more,
 * because a setting that reads like the guarantee and does nothing is worse
 * than no setting at all — the next person would trust the name.
 *
 * With `analytics_storage: 'denied'` GA4 sends cookieless pings instead: no
 * cookie, no device storage, a fresh client id per page load. That is the
 * configuration `/legal/cookies` describes and the reason there is still no
 * banner. The advertising three are denied as well; they are what would make
 * this advertising data, and this product has no ads.
 *
 * Google's behavioural modelling, which is what normally fills the gap left by
 * denied consent, needs traffic thresholds and a consent *signal* from a
 * banner. There is no banner, so there is no modelling, and the numbers are
 * simply the honest undercount described above. That is the trade.
 */
...
export function gaConsentDefaults(): GaConsent {
  return {
    ad_storage: 'denied',
    ad_user_data: 'denied',
    ad_personalization: 'denied',
...
/** The subset of gtag's config this app sets. */
...
export function gaOptions(): GaOptions {
  return {
    // Google signals is cross-device tracking against signed-in Google
    // accounts, and ad personalisation is what makes the data advertising
    // data. Both are refused here and both must also be off in the property.
    // Neither is what keeps the cookie away — see `gaConsentDefaults`.
    allow_google_signals: false,
    allow_ad_personalization_signals: false,

    // gtag would otherwise send a `page_view` from `location.href` the moment
    // it configures — unscrubbed, which on `/consent/approve/<token>` would put
    // a live guardian-consent credential in Google's event store (plan §3,
    // guarantee 4). Pageviews are sent by hand instead, from `gaPageContext`.
    send_page_view: false,
  };
}

/**
 * Where GA is told we are, with the credential taken out.
 *
 * This is `scrubProperties`' opposite number and exists for the same reason.
 * gtag attaches `page_location` to **every** event and reads it from
 * `location.href` unless it is given one, so this is not only about pageviews:
 * without it, a `report_filed` fired from a consent page would carry the token
 * as surely as a pageview would.
 *
 * `page_referrer` is omitted rather than sent empty, because GA treats an empty
 * string as a referrer of "" rather than as absent.
 */
export type GaPageContext = { page_location: string; page_referrer?: string };

export function gaPageContext(href: string, referrer: string): GaPageContext {
  const context: GaPageContext = { page_location: stripQuery(href) };
  const from = stripQuery(referrer);
  if (from.length > 0) context.page_referrer = from;
  return context;
}
