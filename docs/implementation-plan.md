...
**Google Analytics 4 was tried alongside PostHog and removed a day later (2026-09-06 to
2026-09-07, Rachid, in chat, both ways).** Recorded rather than erased, because it is a reasonable
idea that will occur to somebody again, and the reason it fails is not obvious until it has cost a
day.

The ask was the owner's own: he works in GA daily and wanted these numbers in that interface. It
was added beside PostHog rather than replacing it, which is the only reason nothing was lost.

**It cannot work here, and no amount of configuration changes that.** GA4 without a cookie means
Consent Mode with `analytics_storage: 'denied'`. The tag then fires and sends cookieless pings —
which it did, correctly, verified on production — but those pings only ever reach a report through
Google's **behavioural modelling**, and modelling activates only above a threshold of roughly a
thousand daily users with consent **granted**. This product has no consent banner and never will
(the argument is below), so its granted-user count is zero permanently. The threshold is therefore
not "not yet" but "never", and the property showed "No data received from your website yet" with
every hit arriving. The choice is a cookie or an empty dashboard, and:

**The cookie is not open to this product.** It would need consent in the UK and the EU. The UK's
statistical-purposes exception (DUAA 2025 Sch A1 para 5, ICO guidance 2026-04-29) excludes
user-level tracking, and a two-year per-device client id is exactly that. Consent is then the wrong
instrument regardless: UK GDPR Article 8 puts the digital-consent age at 13, so most riders here
cannot lawfully give it, and the ICO Children's Code says this collection should be off by default
under 18. A banner our own users cannot answer buys nothing.

**What it cost while it ran:** Google received a rider's IP address and user agent on every page
...
production — and every one of those checks passed while the thing was worthless. Nobody asked
whether a number would ever appear in the dashboard until after it had shipped.
...
**Superseded in part, 2026-09-18 (Rachid, in chat): "Your tricks" on Home is every tracked trick, learning first.** The heading had said "Your tricks" since 2026-09-17 and the link beside it had counted every stage since the same change, but the grid under them was still the `trying` slice — so a rider with two landed and one being learned read "All 3 of yours" over a single card, with nothing on the screen to say the other two were a tap away. The grid now follows the same question the heading and the link do: has this rider tracked anything at all? Yes means their own tricks; no means suggestions from the library.

Three things the owner settled with it. The **order** is learning first, then the ordinary `STAGES` order — `DASHBOARD_STAGE_ORDER` and `trackedTricksForDashboard` in `@landit/core`, because it is a rule about the product and not a detail of one screen, with difficulty sorting within a stage as `/library?mine=1` already does. The **cap did not move**: four on desktop, the first two on a phone, cut in CSS as before, so Home stays a slice with "All N of yours →" meaning something. And the **stage row is on every one of the rider's own cards** rather than only the ones at `trying`, which is what makes the widening worth having — a trick bumped off Learning moves *down* the grid instead of out of it, and a trick a rider has lost is bumped back down without leaving the screen.

That retires most of what the 2026-09-01 toast was written for, and the toast **stays** all the same: the grid is still the first four, so a rider with a long list can bump a trick to a position off the slice. `page.tsx`, `HomeScreen.tsx`, `WorkingTrick.tsx`, `packages/core/src/rules/library.ts`.
...
The set goes from 135 awards to **297**, and `STICKERS` from 145 to 307. The PNGs were generated by
the owner from these exact ids, so `img` was written before the file existed, and
`apps/web/src/lib/award-art.test.ts` carried a `PENDING_ART` exemption list in the meantime.
**All 162 landed on 2026-09-05 (`chore-badge-art`) and that list is gone** — not emptied, removed,
so there is no longer a place to put a missing badge. "Every award record has a committed file
behind it" is once again unconditional.
