...
| Social | `/crew`, `/join/[code]`, `/riders/[handle]` | Up to 5 owned crews, server-minted invite codes (25 uses / 14 days), crew board + fixed-sentence activity feed, public profiles. Since the app shell rethink (T52) the crew screen is a boxed tab row at every width — **Board · Activity · Members**, the tab kept in `?tab=` — where the board and the feed used to sit side by side; Members is the board's own rows drawn as a roster (avatar, name, sports, who started it) from the same crew-board route, so a rider whose profile is private holds their place there by name exactly as they do on the board, and is still absent from Activity. Invite is in the header, and "Start another" / "Join with a code" are two ghost buttons opening one form each in place of a `<details>` that opened both. A rider profile is the card with its three numbers plus **Landed · Stickers · Videos**, the Videos tab drawn only where the `clips` rule actually returned something — an empty tab would say what `VideoWall` has always refused to say — with "Who sees this" and the report route staying off the row; its back link is "Crew" (§2.3), and a rider's own sticker wall shows twelve badges on it where the old side panel showed six. |
